...
int criarEstruturaAuxiliar(int posicao, int tamanho) {
    ...
    if (vetorPrincipal[indice].elementos != NULL) return JA_TEM_ESTRUTURA_AUXILIAR;

    vetorPrincipal[indice].elementos = (int *)malloc(tamanho * sizeof(int));
    if (vetorPrincipal[indice].elementos == NULL) return SEM_ESPACO_DE_MEMORIA;
...
int inserirNumeroEmEstrutura(int posicao, int valor) {
    ...
    int indice = posicao - 1;
    if (vetorPrincipal[indice].elementos == NULL) return SEM_ESTRUTURA_AUXILIAR;
    if (vetorPrincipal[indice].qtdAtual >= vetorPrincipal[indice].tamanhoTotal) return SEM_ESPACO;
...
int excluirNumeroDoFinaldaEstrutura(int posicao) {
    ...
    int indice = posicao - 1;
    if (vetorPrincipal[indice].elementos == NULL) return SEM_ESTRUTURA_AUXILIAR;
    if (vetorPrincipal[indice].qtdAtual == 0) return ESTRUTURA_AUXILIAR_VAZIA;
...
int getDadosDeTodasEstruturasAuxiliares(int vetorAux[]) {
    ...
    if (cont == 0) return TODAS_ESTRUTURAS_AUXILIARES_VAZIAS;
    return SUCESSO;
}
